...
export class Driver {
  ...
  private setSollByStellarium(chunk:Buffer):void{
    ...
    const {azimuth,altitude} = 
      coordinateCalc.equatorialSpherical2topocentricHorizontal(coords,location,T);
  ...
  private sendPosition(socket:net.Socket):void{
    ...
    const DEC= Math.asin(
        ...
        Math.cos(deg2rad(breite))*Math.cos(deg2rad(this.istPosition.azimuth))*Math.cos(deg2rad(this.istPosition.alt))
        );
    //const stundenwinkel=Math.atan2(Math.cos(deg2rad(this.istPosition.azimuth)))
    console.log(rad2deg(DEC));
  }
}
